...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {    
		...
		gl2.glMaterialfv(GL2.GL_FRONT,GL_AMBIENT_AND_DIFFUSE,new float[]{0,1,0,0.8f},0);
		
		float R = 2;
        int n = 12;
        float[] points_X = new float[n];
        float[] points_Y = new float[n];
        points_X[0] = (float) (R * Math.cos((2.0 * Math.PI * 0) / n));
        points_Y[0] = (float) (R * Math.sin((2.0 * Math.PI * 0) / n));
        ...
        for (int i = 1; i < n; i++) {
            ...
            points_X[i] = (float) (R * Math.cos((2.0 * Math.PI * i) / n));
            points_Y[i] = (float) (R * Math.sin((2.0 * Math.PI * i) / n));
            gl2.glPushMatrix();
            create_Trangle(gl2, points_X[i-1],points_Y[i-1],points_X[i],points_Y[i],0);
            gl2.glPopMatrix();
            gl2.glPushMatrix();
            create_Trangle(gl2, points_X[i-1],points_Y[i-1],points_X[i],points_Y[i],2.5f);
            gl2.glPushMatrix();
        }

        gl2.glPushMatrix();
        create_Trangle(gl2, points_X[n-1],points_Y[n-1],points_X[0],points_Y[0],0);
        gl2.glPopMatrix();
        gl2.glPushMatrix();
        create_Trangle(gl2, points_X[n-1],points_Y[n-1],points_X[0],points_Y[0],2.5f);
        gl2.glPopMatrix();
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl2.glLightfv(GL_LIGHT0, GL_POSITION, new float[]{1,3,0,0},0);
        gl2.glLightfv(GL_LIGHT1, GL_POSITION, new float[]{3,0.1f,0,0},0);
        gl2.glLightfv(GL_LIGHT2, GL_POSITION, new float[]{-3,1,-3,1},0);
        
        set_colors();
        gl2.glLightfv(GL_LIGHT0, GL_SPECULAR,new float[]{red,green,blue,0.1f},0);
        set_colors();
        gl2.glLightfv(GL_LIGHT1, GL_DIFFUSE, new float[]{red,green,blue,0.2f},0);
        set_colors();
        gl2.glLightfv(GL_LIGHT2, GL_DIFFUSE, new float[]{red,green,blue,0},0);      
	}
	
	public void create_Trangle(GL2 gl2, float first_X, float first_Y,float second_X,float second_Y, float height){
        gl2.glBegin(GL_TRIANGLE_FAN);
        gl2.glNormal3f(first_X,1,first_Y);
        gl2.glVertex3f(first_X,0,first_Y);
        gl2.glNormal3f(second_X,1,second_Y);
        gl2.glVertex3f(second_X,0,second_Y);
...
} // end class Lab6 // end nested class MouseHandler
