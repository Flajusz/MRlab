...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		if(objectNumber==1) 
		{
			...
			for (i = 0.0f; i<= (PI * n); i += distance) {
				x = (float) Math.sin(i) * 5;
				z = (float) Math.cos(i) * 3;
				gl2.glColor3f(0f, 1.0f, 0f); 
		...
		else if(objectNumber==2) 
		{
	        ...
	        points_X[0] = (float) (R * Math.cos((PI* 0) / n));
	        points_Y[0] = (float) (R * Math.sin((PI * 0) / n));

	        gl2.glPushMatrix();

	        gl2.glScalef(2,2,2);
	        for (int i = 1; i < n; i++) {
	            points_X[i] = (float) (R * Math.cos((PI* i) / n));
	            points_Y[i] = (float) (R * Math.sin((PI * i) / n));
	            gl2.glPushMatrix();
	            create_Triangle(gl2, points_X[i-1],points_Y[i-1],points_X[i],points_Y[i],0);
	            gl2.glPopMatrix();
	            gl2.glPushMatrix();
	            create_Triangle(gl2, points_X[i-1],points_Y[i-1],points_X[i],points_Y[i],height);
	            gl2.glPushMatrix();
	        }
	        
	        gl2.glPushMatrix();
	        create_Triangle(gl2, points_X[n-1],points_Y[n-1],points_X[0],points_Y[0],0);
	        gl2.glPopMatrix();
	        gl2.glPushMatrix();
	       create_Triangle(gl2, points_X[n-1],points_Y[n-1],points_X[0],points_Y[0],height);
	        gl2.glPopMatrix();
	...
	public void create_Triangle(GL2 gl2, float first_X, float first_Y, float second_X, float second_Y, float height) {
